...
6. **多模型 API 密钥动态配置 (Dynamic Multi-Provider Config)：** 内置可视化的 API 密钥配置面板，支持动态切换 Google Gemini、OpenAI、Anthropic、DeepSeek 等多个大模型服务商的 API Key。配置数据安全地存储在您的本地浏览器中，彻底摆脱频繁修改环境变量文件的繁琐。
7. **全局交互与操作防护 (Global UX & Safety)：** 引入了全局 Toaster 浮动消息提示与危险操作确认弹窗 (Confirm Dialog)，在您删除关键节点或历史画布时为您提供安全的拦截保护，极大地改善了核心交互体验。
8. **工作流全景数据导出 (JSON Export)：** 提供一键全盘将当前工作区完整的高维树状结构化节点映射图，脱水备份导出为结构清晰的 `.json` 格式文件功能。
...
2. **(可选) 默认环境变量权限配置**
   虽然您现在可以直接在网页界面的右上角点击 ⚙️ (设置) 图标去动态配置各大模型的 API 密钥，但如果你想在本地硬编码作为默认 fallback，那么可以在项目根目录下创建一个名为 `.env.local` 的安全认证文件，并填入以下参数：
